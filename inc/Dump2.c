...
//    if zero, the signal is considered low.
    uint32_t num = 0; // # of period measurments
    uint32_t PrevTime = 0; //last rising edge
    uint32_t i = 0; //search index
    uint32_t period = 0; //period btw edges
    uint32_t first = 0; //flag for first rising edge
    uint32_t flag = 0; //flag indicating search for high or low
    uint32_t AvgP = 0; //average period btw rising edges
    uint32_t sum = 0;
    uint32_t DebugCnt = MAXBUF;

    while(i<DebugCnt){ //only want to search through valid entries
        if(flag ==0){ //search for low (0)
            if((DataBuffer[i] & mask) == 0) { //masking out to check for low
                flag = 1; // low found, search for rising edge
            }
        } else { // flag = 1, looking for high signal (rising edge)
            if((DataBuffer[i] & mask) != 0) {
                if(first){    //on first iteration, first will be zero so itll be false and perform the else option which is first = 1
                    period = PrevTime - TimeBuffer[i]; // period calculation
                    sum += period; //period added to sum
                    num ++; //# of periods counter


                } // you never need this again after fist iteration because period calculations occur continuously
                first = 1; //found first rising edge
                PrevTime = TimeBuffer[i];
                flag = 0; //look for next falling edge (0)
            }
        }
        i++;
    }
    if(num == 0){//no periods were found
        return 0;
    }
    return sum / num; //return average period
}
